...
import { parseAppSettings } from '../lib/validation';
...
const Settings: React.FC = () => {
  ...
  const [settings, setSettings] = useState<LocalSettings>({
    ...
    emailAlerts: mockSettings.emailAlerts,
  ...
  const buildDate = useMemo(() => new Date().toLocaleDateString(), []);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const raw = await api.get<unknown>('/settings');
        ...
        if (isMounted) {
          setError('Unable to load settings from the API. Showing local defaults.');
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
  ...
  const handleSettingChange = <K extends keyof LocalSettings>(key: K, value: LocalSettings[K]) => {
    setSettings((prev) => ({
      ...prev,
      [key]: value,
    }));
    ...
    <Layout>
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400">Manage your application preferences and configuration.</p>
        </div>
        ...
        {error ? (
          <InlineAlert
            variant="error"
            title={error}
            description="Check your API URL configuration and network connectivity."
          />
        ) : null}
            ...
            <div className="space-y-6">
              <div className="card p-6">
                ...
                <div className="space-y-3">
                  ...
                  <button className="w-full btn-secondary">Import Settings</button>
                  <button className="w-full btn-danger">Reset to Defaults</button>
                </div>
              </div>
              ...
              <div className="card p-6">
                ...
                <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                  <p>
                    <strong>App Version:</strong> {process.env.REACT_APP_VERSION || '1.0.0'}
                  </p>
                  <p>
